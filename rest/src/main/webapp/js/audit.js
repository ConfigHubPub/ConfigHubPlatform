...
function auditControllerFunc($scope, $http, $stateParams, $httpParamSerializer, $filter, store, contextService, getAuditFunc)
{
    var t = store.get("repoAuditTypes");
    var v = store.get("splitView");
    var range = {
        firstId: 0,
        lastId: 0
    };
        ...
        return {
            ...
            controller:
                ['$scope', '$http', '$stateParams', '$httpParamSerializer', '$filter', 'store', 'contextService',
                    ...
                    {
                        function getPropertyAudit(range, forward)
                        {
                            ...
                            }).then(function successCallback(response)
                            {
                                ...
                                if (response.data.success)
                                {
                                    ...
                                    if ($scope.audit && $scope.audit.length > 0) {
                                        range.firstId = $scope.audit[0].rev;
                                        range.lastId = $scope.audit[$scope.audit.length-1].rev;
                                    }
                                }
                            });
                        }
                        auditControllerFunc($scope, $http, $stateParams, $httpParamSerializer, $filter, store, contextService, getPropertyAudit);
                    }]
        }
    }])
        ...
        return {
            ...
            controller:
                ['$scope', '$http', '$stateParams', '$httpParamSerializer', '$filter', 'store', 'contextService',
                    function($scope, $http, $stateParams, $httpParamSerializer, $filter, store, contextService)
                    {
                        function getRepoAudit(range, forward)
                        {
                            var orderBy = $filter('orderBy');
                            if (!$scope.account || !$scope.repoName)
                                return;
                        ...
                        auditControllerFunc($scope, $http, $stateParams, $httpParamSerializer, $filter, store, contextService, getRepoAudit);
                    }]
        }
    }])
