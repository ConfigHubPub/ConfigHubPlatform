        ...
        for (int i = 0; i < arr.size(); i++)
        {
            ...
            if (entry.has("file"))
            {
                try
                {
                    ...
                    Set<Level> context = ContextParser.parseAndCreateViaApi(contextString,
                                                                            repository,
                                                                            store,
                                                                            appName,
                                                                            changeComment);
                ...
                if (entry.has("values"))
                {
                    ...
                    for (int j = 0; j < values.size(); j++)
                    {
                        ...
                        try
                        {
                            ...
                            Set<Level> levels = ContextParser.parseAndCreateViaApi(context,
                                                                                   repository,
                                                                                   store,
                                                                                   appName,
                                                                                   changeComment);

                            Property property = propertyKey.getPropertyForContext(levels);
                                ...
                                switch (propertyKey.getValueDataType())
                                {
                                    ...
                                    case Map:
                                        if (valueJson.get("value").isJsonNull())
                                            ...
                                            property.setValue(valueJson.get("value").toString(), pass);
                                        break;
